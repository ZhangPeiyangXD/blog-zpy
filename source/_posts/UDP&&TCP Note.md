...
| 特性 | Socket（TCP客户端） | ServerSocket（TCP服务器） | DatagramSocket（UDP） |
|------|-------------------|-------------------------|---------------------|
| 通信协议 | TCP | TCP | UDP |
| 可靠性 | 可靠，有序 | 可靠，有序 | 不可靠，可能乱序/丢失 |
| 连接性 | 面向连接 | 面向连接 | 无连接 |
| 通信模式 | 流（Stream） | 不直接通信，产生Socket | 数据报（Datagram Packet） |
| 主要角色 | 客户端 | 服务器端 | 发送端和接收端（对等） |
| 数据单位 | 字节流（byte stream） | - | 数据报包（DatagramPacket） |
| 类比 | 打电话的主动方 | 电话总机 | 寄送明信片 |

### 核心记忆点

**TCP通信**

- 需要一对搭档：客户端用Socket，服务器端用ServerSocket+Socket
- 像打电话：先接通再说话，保证通话质量
